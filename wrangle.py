...
def wrangle_curriculum_logs(df):
    '''
    Takes in a dataframe of the curriculum logs data, 
    returns a clean and wrangled version of the df.
    '''
    # Combine date and time columns into one column
    df['timestamp'] = df['date'] + ' ' + df['time']
    # Convert new 'timestamp' column to datetime
    df.timestamp = pd.to_datetime(df.timestamp)
